...
#ifndef _TME_CPP
	#define _TME_CPP
// a játékost reprezentáló objektum definíciója
#include <math.h>
#include <time.h>
#include "globals.cpp"
...
// alapértelmezés szerinti billentyü beállítások a scan kód alapján:
int CKey[7]={ 72 , 80 , 75 , 77 , 30 , 31 ,  29 };
#define  NMAX  10         // az orr mozgásának maximális értéke
#include "counter.cpp"
int  K		=20;		// fordulási szög   (166 MHz)
int  STEP  =50;		// lépés távolság
long Rsize	=0;		//(SCR/10);
int  MIN=	143; 		// minimális falmegközelítési távolság
#define MAXFLOWERS 7 // az összegyûjtendõ virágok száma
#include "Tomato.cpp"
#define GUN_Tomato 0  // lövedék típus tömb azonosítója
#define GUN_NUM 1     // lövedék típusok száma
class TMotor;
class	TMe: TObj , TTimer ,public TPozition
{
	TMap * Map;                   // térkép mutató
	TMotor *Motor;                // motor  mutatója
	TScrObj * Head;               // a fej Screen obj. mutatója
	TScrObj * Nozi;               // az orr Screen obj. mutatója
	TScrObj * FlowPic;            // a virágok számát jelzó Screen obj.-ok mutatója
	TScrObj * Mission;            // a küldetés teljesítését jelzõ Screen obj. mutatója
	TCounter *Counter;            // a lövedék szám kijelzõ mutatója
	TEnergy * Energy;             // az energia kijelzõ mutaója
	void Left()		;              // fordul balra
	void Right()	;              // fordul jobbra
	void Forward() ;              // egy lépés elõre
	void Backward();              // egy lépés hátra
	POINT HeadPoz;                // a fej poziciója a képernyõn
	int HeadTop;                  // a fej aktuális emelése
	POINT NoziPoz;                // az orr poziciója a képernyõn
	int NoziTop,NoziShift;        // az orr eltolásai vizszintes ill. függ. irányban
	BOOL Shuting;                 // lövés folyamatban jelzõ
	long LastTime;                // a legutóbbi lövedék induásakor mért idõ
	int Gun[ GUN_NUM ];           // a fegyverek típusai
	int GunNow;                   // a jelenlegi fegyver azonosító
	int Flowers;                  // az összegyûjtött virágok száma
	int MissionCounter;           // számláló a pálya befelyezésétõl az játék végéig
	BOOL TimeSet;                 // segéd változó az idõzítõ megszüntetéséhez
	BOOL Protection;              // védelem a pálya teljesítésekor
	int Level;                    // az aktuális pálya száma
	int ShutCount;                // a legutóbbi lövedék induásától mért idõ
	long Time; // :)              // a játék kezdetekort mért idõ
public:
	static char UserName[];       // a játékos megadott neve
	static long UserTime;         // a játékos álltal teljesített idõ
	int Trapp;                    //
	virtual void Proc();
